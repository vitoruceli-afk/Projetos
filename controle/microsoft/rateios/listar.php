...
$rateios  = RateioHistorico::listar('microsoft', $busca);
...
<form method="GET" class="filtro-bar d-flex flex-wrap gap-2 align-items-center mb-3">
    ...
    <a href="<?= url('microsoft/rateios/listar.php') ?>" class="btn btn-outline-secondary">Limpar</a>
</form>
        ...
        <table class="table table-bordered table-hover align-middle">
            ...
            <?php foreach ($rateios as $r): ?>
                <tr>
                    ...
                    <td>
                        <a href="<?= url('microsoft/rateios/ver.php?id=' . $r['id']) ?>"
                           class="btn btn-info btn-sm">Consultar</a>
                        <a href="<?= url('microsoft/rateios/exportar.php?id=' . $r['id']) ?>"
                           class="btn btn-success btn-sm">CSV</a>
                        <a href="<?= url('microsoft/rateios/enviar.php?id=' . $r['id']) ?>"
                           class="btn btn-primary btn-sm"
                           onclick="return confirm('Enviar este rateio por e-mail para a lista Microsoft?')">
                           <i class="bi bi-envelope"></i> E-mail</a>
                        <?php if ($ehAdmin): ?>
                            <a href="<?= url('microsoft/rateios/excluir.php?id=' . $r['id']) ?>"
